...
module.exports.run = (client, message, args) => {
    const page = functions.pages(message.guild.members.cache.filter(m => m.user.username.toLowerCase().includes(args[0].toLowerCase())).map(u => u.user.tag + '  |  ' + u.user.id), 15, args[1])
